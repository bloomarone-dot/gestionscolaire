...
def delete_note(note_id: int, db: Session = Depends(get_db), ctx: TenantContext = Depends(require_tenant)):
    try:
...
def create_periode(payload: PeriodeIn, db: Session = Depends(get_db), ctx: TenantContext = Depends(require_tenant)):
