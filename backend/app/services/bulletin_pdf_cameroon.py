...
def _row_from_spans(cells: list[tuple]) -> list:
    """Construit une ligne 9 colonnes : [(contenu, colspan), ...]."""
    row: list = []
...
def build_cameroon_bulletin_pdf(bulletin: dict) -> bytes:
    ...
    def add_row(cells: list[tuple], spans: list[int], style_key: str | None = None):
        ...
        body_rows.append(_row_from_spans(cells))
    ...
    if lang == "en":
        add_row([
            (_p(f"<b>Name:</b> {eleve_full}", 7), 3),
            (_p(f"<b>Class:</b> {bulletin.get('classe', '—')}", 7), 2),
            (_p(f"<b>Sex:</b> {bulletin.get('eleve_sexe', '—')}", 7), 2),
            (_p(f"<b>Class Enrollment:</b> {bulletin.get('effectif', '—')}", 7), 2),
        ], [3, 2, 2, 2], "info")
        add_row([
            (_p(f"<b>Repeater:</b> {bulletin.get('redoublant', 'NO')}", 7), 2),
            (_p(f"<b>Unique ID:</b> {bulletin.get('matricule', '—')}", 7), 3),
            (_p(f"<b>Term:</b> {bulletin.get('term_label', '')}", 7, True), 2),
            (_p(f"<b>Year:</b> {bulletin.get('annee_scolaire', '—')}", 7, True), 2),
        ], [2, 3, 2, 2], "info")
    else:
        add_row([
            (_p(f"<b>NOM:</b> {eleve_full}", 7), 3),
            (_p(f"<b>Classe:</b> {bulletin.get('classe', '—')}", 7), 2),
            (_p(f"<b>Effectif:</b> {bulletin.get('effectif', '—')}", 7), 2),
            (_p(f"<b>Redoublant:</b> {bulletin.get('redoublant', 'NON')}", 7), 2),
        ], [3, 2, 2, 2], "info")
        add_row([
            (_p(f"<b>Serie:</b> {bulletin.get('classe_serie', '—')}", 7), 2),
            (_p(f"<b>Matricule:</b> {bulletin.get('matricule', '—')}", 7), 3),
            (_p(f"<b>{bulletin.get('term_label', '')}</b>", 7, True), 2),
            (_p(f"<b>Annee</b> {bulletin.get('annee_scolaire', '—')}", 7, True), 2),
        ], [2, 3, 2, 2], "info")
    ...
    if lang == "en":
        add_row([
            (_p(f"<b>TOTAL</b><br/>{bulletin.get('total_coef', 0)}", 7, True, "CENTER"), 2),
            (_p(f"<b>Total marks</b><br/>{bulletin.get('total_points', 0)}", 7, True, "CENTER"), 2),
            (_p(f"<b>Class Average</b><br/>{bulletin.get('moyenne_classe', '—')}", 7, True, "CENTER"), 2),
            (_p(f"<b>Term Average</b><br/>{bulletin.get('moyenne_generale', '—')}", 7, True, "CENTER"), 1),
            (_p(f"<b>Position</b><br/>{bulletin.get('rang_label', '—')}", 7, True, "CENTER"), 1),
            (_p(f"<b>Remark</b><br/>{bulletin.get('decision', '—')}", 7, True, "CENTER"), 1),
        ], [2, 2, 2, 1, 1, 1], "summary")
        add_row([
            (_p("<b>Absences (hours)</b>", 7), 2),
            (_p(str(bulletin.get("absences", 0)), 7, align="CENTER"), 1),
            ("", 1),
            (_p(f"<b>Sanctions</b><br/>{bulletin.get('sanctions', '')}", 7), 2),
            (_p(f"<b>Observation</b><br/>{bulletin.get('observation', '')}", 7), 2),
            ("", 1),
        ], [2, 1, 1, 2, 2, 1], "summary")
        sig_labels = ["PARENTS/GUARDIANS", "S.D.M", "PRINCIPAL", "DATE"]
    else:
        add_row([
            (_p(f"<b>TOTAL</b><br/>Coef: {bulletin.get('total_coef', 0)}", 7, True, "CENTER"), 2),
            (_p(f"Notes: {bulletin.get('total_points', 0)}", 7, True, "CENTER"), 2),
            (_p(f"<b>Moyenne de la classe</b><br/>{bulletin.get('moyenne_classe', '—')}", 7, True, "CENTER"), 2),
            (_p(f"<b>Moyenne</b><br/>{bulletin.get('moyenne_generale', '—')}", 7, True, "CENTER"), 1),
            (_p(f"<b>Absences</b><br/>{bulletin.get('absences', 0)}", 7, True, "CENTER"), 1),
            (_p(f"<b>Rang</b><br/>{bulletin.get('rang_label', '—')}", 7, True, "CENTER"), 1),
        ], [2, 2, 2, 1, 1, 1], "summary")
        add_row([
            (_p(f"<b>Decision</b><br/>{bulletin.get('decision', '—')}", 7, True, "CENTER"), 2),
            (_p(f"<b>OBSERVATION</b><br/>{bulletin.get('observation', '')}", 7), 4),
            ("", 3),
        ], [2, 4, 3], "summary")
    ...
    add_row([("", 3), ("", 2), ("", 2), ("", 2)], [3, 2, 2, 2], "sig_body")
