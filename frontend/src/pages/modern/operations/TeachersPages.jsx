...
export function PersonnelCreatePage() {
  ...
  async function submit(event) {
    ...
    if (!normalizePhone(form.phone)) {
      setNotice("Numéro de téléphone invalide.");
      return;
    }
    try {
      if (isTeacher) {
        ...
        if (form.assignment && created?.id) {
          ...
          try {
            ...
            setNotice(
              `Enseignant créé, mais l'affectation a échoué : ${assignErr.message || "erreur inconnue"}. `
              + "Vous pouvez l'affecter depuis Matières.",
            );
            return;
          }
        }
      } else {
        if (!normalizePhone(form.phone2)) {
          setNotice(
            "Un deuxième téléphone valide est obligatoire pour la Direction.",
          );
          return;
        }
  ...
  return (
    <>
      ...
      {subjectsError && (
        <Notice message={subjectsError} tone="amber" />
      )}
      <Card className="p-5">
