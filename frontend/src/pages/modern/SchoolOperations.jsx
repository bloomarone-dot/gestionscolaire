...
export function OperationalTeachersPage() {
  ...
  const view = searchParams.get('fonction') === 'direction' ? 'direction' : 'enseignant';
  ...
  const pageTitle = view === 'direction' ? 'Direction / Administration' : 'Enseignants';
  const pageDesc = view === 'direction'
    ? 'Censeurs, directeurs d\'études et surveillants.'
    : 'Enseignants et matières assignées par classe.';
  const createHref = view === 'direction' ? '/app/teachers/nouveau?fonction=direction' : '/app/teachers/nouveau';
  ...
  return (
    <>
      <PageHeader title={pageTitle} description={pageDesc}
        actions={<Link to={createHref}><Button><UserPlus size={16} /> Nouveau membre</Button></Link>} />
  ...
  return (
    <>
      <PageHeader title="Nouveau membre du personnel" breadcrumb="Personnel" description="Connexion par téléphone ; email facultatif. Direction = 2 téléphones obligatoires."
        actions={<Link to={listHref}><Button variant="secondary">Retour à la liste</Button></Link>} />
      ...
      <Card className="p-5">
        <form className="grid gap-4 md:grid-cols-2" onSubmit={submit}>
          ...
          <div className="md:col-span-2 flex justify-end gap-2">
            <Link to={listHref}><Button type="button" variant="secondary">Annuler</Button></Link>
...
export function OperationalClassesPage() {
  ...
  return (
    <>
      ...
      ]} rows={rows} rowClassName={(row) => (
        String(row.id) === highlightId ? 'bg-blue-50 ring-1 ring-inset ring-blue-200' : 'hover:bg-slate-50'
    ...
    <>
      ...
      <DataTable title="Registre des élèves" columns={[
        ...
        { key: 'className', label: 'Classe', render: (row) => (
          row.classe_id
            ? <Link to={`/app/classes?highlight=${row.classe_id}`} className="font-semibold text-blue-600 hover:underline">{row.className}</Link>
            : row.className
        ) },
        { key: 'sexe', label: 'Sexe' },
...
function BulletinsWorkspace({ professor = false }) {
  ...
  const loadClassData = useCallback(async (classId = selectedClass) => {
    ...
    const [nextStudents, nextBulletins] = await Promise.all([
      ...
      api.fetchClasseBulletins(classId, trimestre, scope).then((data) => data.bulletins || data || []).catch(() => []),
    ]);
    setStudents(nextStudents);
  ...
  async function preview(student) {
    try {
      const data = await api.fetchEleveBulletin(student.id, trimestre, 'cameroon', scope);
  ...
  return (
    <>
      <PageHeader title={professor ? 'Bulletins de mes eleves' : 'Bulletins'} description="Generation, apercu, publication et export PDF des bulletins trimestriels et annuels." />
      <Notice message={notice} tone={notice.includes('succes') ? 'emerald' : 'amber'} />
      ...
      <DataTable title="Eleves" columns={[
        ...
        <div className="flex justify-end gap-2">
          ...
          {!professor && <Button variant="secondary" onClick={() => api.exportEleveBulletinPdf(row.id, trimestre, 'auto', null, scope)}><Download size={16} /> PDF</Button>}
        </div>
      )} />
        ...
        <Card className="p-5">
          ...
          {selectedBulletin ? (
            <div className="mt-4 space-y-2 text-sm text-slate-600">
              ...
              <p><strong>Rang:</strong> {selectedBulletin.rang_general || selectedBulletin.rang_label || selectedBulletin.rang || '-'}</p>
              {selectedBulletin.decision && <p><strong>Decision:</strong> {selectedBulletin.decision}</p>}
              <Textarea readOnly value={JSON.stringify(selectedBulletin.subjects || selectedBulletin.details_notes || selectedBulletin.details_matieres || [], null, 2)} rows={8} />
            </div>
