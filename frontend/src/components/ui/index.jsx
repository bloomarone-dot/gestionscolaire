...
export function Table({ columns, rows, renderActions, rowClassName }) {
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-slate-200 text-sm">
        ...
        <tbody className="divide-y divide-slate-100 bg-white">
          {rows.map((row) => (
            <tr key={row.id || row.name} className={rowClassName?.(row) || 'hover:bg-slate-50'}>
...
export function DataTable({ title, description, actions, filters, columns, rows, renderActions, rowClassName }) {
  return (
    <Card>
      ...
      <Table columns={columns} rows={rows} renderActions={renderActions} rowClassName={rowClassName} />
    </Card>
  );
}
