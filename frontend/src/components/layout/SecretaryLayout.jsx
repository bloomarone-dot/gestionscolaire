...
import { Bell, ChevronLeft, ClipboardList, GraduationCap, LogOut, Menu, Search, X } from 'lucide-react';
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex flex-col border-r border-slate-200 bg-white transition-all duration-300 ${collapsed ? 'lg:w-20' : 'lg:w-72'} ${open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} w-72`}>
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-4">
          <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-600 text-white"><ClipboardList size={22} /></span>
            {!collapsed && (
              <div>
                <p className="text-sm font-extrabold">EduGestion</p>
                <p className="text-xs text-slate-500">Secrétariat</p>
              </div>
            )}
          </div>
