      ...
      <div className={`transition-all duration-300 ${collapsed ? 'lg:pl-20' : 'lg:pl-72'}`}>
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6">
            ...
            {user?.role === 'superadmin' && <SchoolSelector />}
        ...
        <main className="px-4 py-6 sm:px-6 lg:px-8">
          {user?.role === 'superadmin' && selectedSchool?.name && (
            <div className="mb-5 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600">
              Mode superadmin — établissement : <strong className="text-slate-900">{selectedSchool.name}</strong>
            </div>
          )}
          <Outlet />
        </main>
