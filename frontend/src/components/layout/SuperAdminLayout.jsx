...
const nav = [
  ...
  { to: '/superadmin/referentiel', label: 'Referentiel national', icon: Library },
      ...
      <div className={`transition-all duration-300 ${collapsed ? 'lg:pl-20' : 'lg:pl-72'}`}>
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6">
            ...
            <form className="hidden max-w-xl flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 md:flex" onSubmit={handleSearch}>
              ...
              <input className="w-full bg-transparent text-sm outline-none placeholder:text-slate-400" placeholder="Recherche: etablissement, admin..." value={query} onChange={(e) => setQuery(e.target.value)} />
            </form>
              ...
              <div className="relative">
                ...
                {notificationsOpen && (
                  <div className="absolute right-0 mt-2 w-80 rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
                    ...
                    {['Nouvel etablissement a valider', 'Compte admin a verifier', 'Services operationnels'].map((item) => (
                      <button key={item} type="button" className="block w-full rounded-lg px-2 py-2 text-left text-sm text-slate-600 hover:bg-slate-50">{item}</button>
                    ))}
                  </div>
                )}
              </div>
              <div className="relative">
                <button type="button" className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2 text-left hover:bg-slate-50" onClick={() => setUserMenuOpen((value) => !value)}>
                  <Avatar name={name} />
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
                    <button type="button" className="block w-full rounded-lg px-3 py-2 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50" onClick={() => navigate('/superadmin/settings')}>Parametres plateforme</button>
                    <button type="button" className="block w-full rounded-lg px-3 py-2 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50" onClick={() => navigate('/superadmin/schools')}>Etablissements</button>
                    <button type="button" className="block w-full rounded-lg px-3 py-2 text-left text-sm font-semibold text-rose-600 hover:bg-rose-50" onClick={handleLogout}>Deconnexion</button>
                  </div>
                )}
              </div>
            </div>
        ...
        <main className="px-4 py-6 sm:px-6 lg:px-8">
          <div className="mb-5 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
            <strong className="text-slate-900">Role superadmin :</strong> creer les etablissements et leurs administrateurs.
            Les enseignants, eleves, classes et assignations sont geres par chaque admin d&apos;etablissement apres connexion.
          </div>
          <Outlet />
        </main>
      </div>
