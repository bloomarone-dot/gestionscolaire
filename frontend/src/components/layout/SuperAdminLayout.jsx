...
import {
  ...
  Settings, ShieldCheck, UserPlus, X, School,
} from 'lucide-react';
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6">
            ...
            <div className="ml-auto flex items-center gap-2">
              {selectedSchool?.id && (
                <Button variant="secondary" className="hidden sm:inline-flex" onClick={() => navigate('/app/dashboard')}>
                  <School size={16} /> Gérer {selectedSchool.name}
                </Button>
              )}
          ...
          {selectedSchool?.id ? (
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
              <p className="text-sm text-blue-900">
                Établissement actif : <strong>{selectedSchool.name}</strong> — classes, élèves, matières et paramètres bulletin.
              </p>
              <Button onClick={() => navigate('/app/dashboard')}>
                <School size={16} /> Ouvrir la gestion
              </Button>
            </div>
          ) : (
            <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              Sélectionnez un établissement en haut à gauche pour accéder aux classes, élèves, matières et paramètres du bulletin.
            </div>
          )}
          <Outlet />
        </main>
