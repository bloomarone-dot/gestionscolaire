  ...
  return (
    <tr className="bulletin-national-row">
      <td colSpan={sideSpan} className="national-side national-en">
        {enLines.map((line, i) => (
          <div key={i} className={i === 5 ? 'school-name' : i === 6 ? 'motto' : ''}>{line}</div>
        ))}
      </td>
...
function CameroonBulletinView({ bulletin, officialOnly = false }) {
    ...
    <div className="bulletin-official-sheet" style={themeStyle}>
          ...
          <tbody>
            ...
            {isEn ? (
              ...
              <>
                ...
                <tr className="bulletin-info-row bulletin-info-row-b">
                  <td colSpan={infoRow2[0]}><span className="cell-label">EFFECTIF:</span> {bulletin.effectif || '—'}</td>
                  <td colSpan={infoRow2[1]}><span className="cell-label">Redoublant:</span> {bulletin.redoublant || 'NON'}</td>
                  <td colSpan={infoRow2[2]}><span className="cell-label">Série:</span> {bulletin.classe_serie || '—'}</td>
                  <td colSpan={infoRow2[3]} />
                </tr>
                <tr className="bulletin-info-row bulletin-info-row-c">
                  <td colSpan={infoRow2[0]}><span className="cell-label">MATRICULE:</span> {bulletin.matricule || '—'}</td>
                  <td colSpan={infoRow2[1]}><strong>{bulletin.term_label}</strong></td>
                  <td colSpan={infoRow2[2] + infoRow2[3]}><span className="cell-label">ANNÉE:</span> <strong>{bulletin.annee_scolaire || '—'}</strong></td>
                </tr>
              </>
            )}

            <tr className="bulletin-grades-header">
              <th>{isEn ? 'SUBJECTS' : 'MATIÈRES'}</th>
              {isAnnual
              ...
              <th>Appre.</th>
              <th>{isEn ? "Teacher's sign." : 'Professeur M./Mme'}</th>
            </tr>
            ...
            {isEn ? (
              ...
              <>
                <tr className="bulletin-summary-row">
                  <td colSpan={summarySpans[0]}><span className="cell-label">TOTAL</span><br />{bulletin.total_coef}</td>
                  <td colSpan={summarySpans[1]}><span className="cell-label">Notes</span><br />{bulletin.total_points}</td>
                  <td colSpan={summarySpans[2]}><span className="cell-label">MOYENNE DE CLASSE</span><br />{bulletin.moyenne_classe}</td>
                  <td colSpan={summarySpans[3]}><span className="cell-label">MOYENNE DU TRIMESTRE</span><br />{bulletin.moyenne_generale}</td>
                  <td colSpan={summarySpans[4]}><span className="cell-label">SANCTIONS</span><br />{bulletin.sanctions || '0'}</td>
                  <td colSpan={summarySpans[5]}>0</td>
                </tr>
                <tr className="bulletin-summary-row">
                  <td colSpan={summarySpans[0]}><span className="cell-label">Absences (heures)</span><br />{bulletin.absences ?? 0}</td>
                  <td colSpan={summarySpans[1]}>0</td>
                  <td colSpan={summarySpans[2]}><span className="cell-label">RANG</span><br />{bulletin.rang_label || bulletin.rang || '—'}</td>
                  <td colSpan={summarySpans[3]}><span className="cell-label">EFFECTIF</span><br />{bulletin.effectif || '—'}</td>
                  <td colSpan={summarySpans[4]}><span className="cell-label">DÉCISION</span><br />{bulletin.decision}</td>
                  <td colSpan={summarySpans[5]} />
                </tr>
                <tr className="bulletin-summary-row bulletin-observation-row">
                  <td colSpan={nCols}><span className="cell-label">OBSERVATION</span><br />{bulletin.observation || ''}</td>
                </tr>
                <tr className="bulletin-sig-row">
      ...
      {school?.next_term && (
        <p className="bulletin-next-term">
          {isEn ? 'Next term re-opens' : 'Prochaine rentrée'}: {school.next_term}
        </p>
      )}
    </div>
  );
...
export default function BulletinDetail({
  ...
  return (
    <div className={`bulletin-detail-card${officialOnly ? ' bulletin-detail-official' : ''}`}>
      ...
      {isCameroon ? (
        <CameroonBulletinView bulletin={bulletin} officialOnly={officialOnly} />
      ) : (
